...
void Reserve_INSERT(reserve_p* T, char* id, int dist, int cost, int period) { 
...
void Reserve_DELETE(reserve_p *T, char* id)
{
	reserve_p target = *T;
	reserve_p temp = NULL;
	reserve_p x = NULL;
...
void r_inorder(reserve_p* r_T, reserve_p s) {
	if (s != (*r_T)->nil) {
		r_inorder(r_T, s->left);
		printf("%s[%d]\n", s->id, s->cost);
		r_inorder(r_T, s->right);
	}
}
